...
@Directive({
  selector: '[appPhoneCardKeyboard]'
})
export class PhoneCardKeyboardDirective {
  ...
  private getSiblingCards(): HTMLElement[] {
    ...
    if (!parent) return [];
    return Array.from((parent as Element).querySelectorAll<HTMLElement>('[appPhoneCardKeyboard], [role="listitem"][tabindex]'));
  }
