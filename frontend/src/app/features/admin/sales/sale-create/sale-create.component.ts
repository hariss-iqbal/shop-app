  ...
  private addProductToCart(product: any): void {
    ...
    try {
      ...
      const minimalProduct: Product = {
        ...
        model: product.model,
        modelId: product.modelId ?? product.model_id ?? null,
        modelName: product.modelName ?? null,
        displayName: product.displayName ?? [product.modelName ?? product.model, product.storageGb ?? product.storage_gb ? `${product.storageGb ?? product.storage_gb}GB` : ''].filter(Boolean).join(' '),
