...
import { ProductImage, ImageUploadState } from '../../../../models/product-image.model';
...
export class ProductImageUploadComponent implements OnInit, OnDestroy {
  @Input() productId: string | null = null;
  @Output() imagesChanged = new EventEmitter<ProductImage[]>();
  ...
  async loadImages(): Promise<void> {
    ...
    try {
      const response = await this.productImageService.getImagesByProductId(this.productId);
  ...
  private async processFiles(files: File[]): Promise<void> {
    if (!this.productId) {
      this.toastService.warn('Warning', 'Please save the product record before uploading images');
      return;
    }
    ...
    for (const file of files) {
      const validation = this.productImageService.validateFile(file);
      if (validation.valid) {
    ...
    const uploadedImages = await this.productImageService.uploadMultipleImages(
      this.productId,
      validFiles,
      (fileName, progress) => {
  ...
  async onImageDrop(event: CdkDragDrop<ProductImage[]>): Promise<void> {
    ...
    try {
      ...
      await this.productImageService.reorderImages(this.productId, { imageIds });
      this.emitImagesChanged();
    } catch (error) {
  ...
  async onSetPrimary(image: ProductImage): Promise<void> {
    this.actionInProgress.set(true);
    try {
  ...
  async onDelete(image: ProductImage): Promise<void> {
